...
module.exports = async (ctx, next) => {
  ...
  if (permission.policy) {
    await strapi.plugins['users-permissions'].config.policies[
      permission.policy
    ](ctx, next);
  } else {
    // Execute the action.
  ...
  if (
    ...
    !ctx.state.permissions.skipAttributePermissions
  ) {
    ctx.body = await strapi.plugins['users-permissions'].services.serialize.SerializePlainObject(ctx, ctx.body);
  }
};
